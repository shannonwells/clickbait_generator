...
describe Clickraw do
  ...
  let(:empty_results) {{"photos": {"page": "1", "pages": "0", "perpage": "1", "total": "0", "photo": []}, "stat": "ok"}}

  subject(:clickraw) {described_class.run(taglist)}
  ...
  describe "flickr_url" do
    ...
    subject {described_class.new("doesn't matter").flickr_url(photo_id)}
  ...
  describe "flickr_user" do
    ...
    subject {described_class.new("dummy text").flickr_user(owner_id)}
  ...
  describe "run" do
    subject {described_class.run(["kitten", "water"])}

    context "when a match is found" do
      before do
        stub_search search_results
        stub_get_people_info
        stub_get_photo_info
      end
  ...
  def stub_get_people_info
    body_params = default_body_params.merge("method" => "flickr.people.getInfo", "user_id" => owner_id)

    stub_request(:post, "https://api.flickr.com/services/rest/").with(:body => body_params).
        to_return(:status => 200, :body => JSON.generate(user_info), :headers => {})
  end

  def stub_get_photo_info
    body_params = default_body_params.merge("method" => "flickr.photos.getInfo", "photo_id" => photo_id)
  ...
  def stub_search(result)
    body_params = default_body_params.
        merge("method" => "flickr.photos.search",
              "per_page" => "1",
              "sort" => "relevance",
              "text" => "kitten water").
        merge(default_search_params)
